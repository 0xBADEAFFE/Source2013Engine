	...
	*pTo = 0;
	...
	if ( !suppress )
	{
		...
		if ((spewType != SPEW_LOG) || (sv.GetMaxClients() == 1))
		{
			...
			default:
				{
					color.SetRawColor( GetSpewOutputColor()->GetRawColor() );
				}
				break;
			}
...
public:
	...
	virtual bool			ReplayInit( CreateInterfaceFn fnReplayFactory ) { return false; /*DUMMY*/ }
	...
	virtual void			GameServerSteamAPIActivated( void ) { /*DUMMY*/ };

	// Called after the steam API has been shutdown post-level startup
	virtual void			GameServerSteamAPIShutdown( void ) { /*DUMMY*/ };

	virtual void			SetServerHibernation( bool bHibernating ) { /*DUMMY*/ };

	// interface to the new GC based lobby system
	virtual IServerGCLobby *GetServerGCLobby() { return NULL; /*DUMMY*/ }

	// Return override string to show in the server browser
	// "map" column, or NULL to just use the default value
	// (the map name)
	virtual const char *GetServerBrowserMapOverride() { return NULL; /*DUMMY*/ }
	...
	if ( g_ServerFactory )
	{
		...
		if ( !serverGameDLL )
		{
			...
			if ( !serverGameDLL )
			{
				...
				serverGameDLL = (IServerGameDLL*)g_ServerFactory("ServerGameDLL004", NULL);
