...
{
public:
	virtual void OnDynamicModelsStringTableChange( int nStringIndex, const char *pString, const void *pData ) { /*DUMMY*/ };
	...
	virtual void					OnLevelChange() { /*DUMMY*/ };;

	virtual int						GetModelClientSideIndex( const char *name ) const { return NULL; /*DUMMY*/ }

	// Returns index of model by name, dynamically registered if not already known.
	virtual int						RegisterDynamicModel( const char *name, bool bClientSide ) { return NULL; /*DUMMY*/ }

	virtual bool					IsDynamicModelLoading( int modelIndex ) { return false; /*DUMMY*/ }

	virtual void					AddRefDynamicModel( int modelIndex ) { /*DUMMY*/ };
	virtual void					ReleaseDynamicModel( int modelIndex ) { /*DUMMY*/ };

	// Registers callback for when dynamic model has finished loading.
	// Automatically adds reference, pair with ReleaseDynamicModel.
	virtual bool					RegisterModelLoadCallback( int modelindex, IModelLoadCallback* pCallback, bool bCallImmediatelyIfLoaded = true ) { return false; /*DUMMY*/ };
	virtual void					UnregisterModelLoadCallback( int modelindex, IModelLoadCallback* pCallback ) { /*DUMMY*/ };
...
const model_t *CModelInfo::FindOrLoadModel( const char *name )
{
