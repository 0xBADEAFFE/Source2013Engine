...
void CClientState::CheckFileCRCsWithServer()
{
	...
	CUnverifiedFileHash crcFiles[nBatchSize];
	int count = g_pFileSystem->GetUnverifiedFileHashes( crcFiles, ARRAYSIZE( crcFiles ) );
	if ( count == 0 )
		return;
	...
	for ( int i=0; i < count; i++ )
	{
		...
		crcCheck.m_CRC = crcFiles[i].m_FileHash.m_crcIOSequence;
