...
void CL_HandlePureServerWhitelist( CPureServerWhitelist *pWhitelist )
{
	...
	g_pFileSystem->RegisterFileWhitelist( (IPureServerWhitelist*)pWhitelist, &pAllowFromDiskList );
	...
	if ( pWhitelist )
	{
		...
		// Handle materials..
		materials->ReloadFilesInList( pWhitelist->GetForceMatchList() );
...
#endif
			if ( nRet ) // if they don't own HL2 this must be the demo!
			{
				s_bIsHL2Demo = true;
			}
		}
...
void CL_InitPortalDemoFlag()
{
	...
	if ( !initialized )
	{
		if ( SteamApps() && !Q_stricmp( COM_GetModDirectory(), "portal" ) && g_pFileSystem->IsSteam() )
		{
			...
			if ( VCRGetMode() != VCR_Playback )
				nRet = SteamApps()->BIsSubscribedApp( 400 );
#if !defined( NO_VCR )
			VCRGenericValue( "e", &nRet, sizeof( nRet ) );
#endif
			if ( nRet ) // if they don't own HL2 this must be the demo!
