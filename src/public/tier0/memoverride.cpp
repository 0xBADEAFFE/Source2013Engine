...
const char* g_pszModule = MKSTRING(DLLNAME);
...
extern "C"
{
...
#if _MSC_VER <= 1800
	void* __cdecl _calloc_base(size_t nSize)
	{
...
#if _MSC_VER <= 1800
	void* __cdecl _recalloc_base(void* pMem, size_t nSize)
	{
	...
	{
		return _recalloc_base(ptr, 0, size * count);
	}
	...
	size_t __cdecl _msize_base(void* pMem)
	{
	...
	size_t __cdecl msize(void* pMem)
	{
		return g_pMemAlloc->GetSize(pMem);
	}
...
#undef _crtDbgFlag
	...
	int* __cdecl AFNAME(_crtDbgFlag)(void)
	{
		return AFRET(_crtDbgFlag);
	}

#ifdef _crtBreakAlloc
#undef _crtBreakAlloc
#endif

	long _crtBreakAlloc;      /* Break on this allocation */
	long* __cdecl AFNAME(_crtBreakAlloc) (void)
	{
