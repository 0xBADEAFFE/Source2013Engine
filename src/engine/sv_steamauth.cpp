...
void CSteam3::Activate()
{
	...
	if ( ipname.IsValid() )
	{
		...
		{
			m_unIP = htonl( ipaddr.GetIPHostByteOrder() );
		}
	}
	...
	case IDTYPE_VALVE:
		{
			return ( id1.steamid==id2.steamid && 
					 id1.steamid.ConvertToUint64() == id2.steamid.ConvertToUint64() );
		}
	default:
		break;
	}
...
// Purpose: 
//-----------------------------------------------------------------------------
void CSteam3::OnBeginLogonRetry( SteamServersConnected_t *pBeginLogonRetry )
{
}
...
void CSteam3::OnGSClientDenyHelper( CBaseClient *cl, EDenyReason eDenyReason, const char *pchOptionalText )
{
	switch ( eDenyReason )
	{
		...
		case k_EDenySteamValidationStalled:
			if ( BLanOnly() ) 
			{
				cl->m_NetworkID.steamid.Set(1, k_EUniverseInternal, k_EAccountTypeIndividual);
...
bool CSteam3::NotifyClientConnect( CBaseClient *client, uint32 unUserID, netadr_t & adr, const void *pvCookie, uint32 ucbCookie )
{
	...
	bool bRet = SteamGameServer()->SendUserConnectAndAuthenticate(adr.GetIPHostByteOrder(), NULL, NULL, (CSteamID *)unUserID);
#else
	bool bRet = false;
#endif
	SendUpdatedServerDetails();
...
bool CSteam3::NotifyLocalClientConnect( CBaseClient *client )
{
	...
	if ( SteamGameServer() )
		client->SetSteamID( SteamGameServer()->CreateUnauthenticatedUserConnection() );
	else
		return false;
#endif
	SendUpdatedServerDetails();
...
void CSteam3::SendUpdatedServerDetails()
{
	if ( !BIsActive() )
		return;
#ifndef NO_STEAM
//	SteamGameServer()->SetMaxPlayerCount(sv.GetNumClients());
