							    ...
							    const char *name, const char *password, const char *hashedCDkey, int cdKeyLen )
{
	...
	client->Connect( name, nNextUserID, netchan, false, 0 );
...
bool CBaseServer::ProcessConnectionlessPacket(netpacket_t * packet)
{
	...
	switch ( c )
	{
		...
		default:
		{
			...
			else if ( SteamGameServer() && Steam3Server().IsMasterServerUpdaterSharingGameSocket() )
			{
				SteamGameServer()->HandleIncomingPacket(
					packet->message.GetBasePointer(), 
					packet->message.TotalBytesAvailable(),
					BigLong( packet->from.GetIPHostByteOrder() ),
					packet->from.GetPort()
					);
...
void CBaseServer::ReplyChallenge(netadr_t &adr)
{
	...
	if ( authprotocol == PROTOCOL_STEAM )
	{
		...
		/*
		if ( !SteamGameServer()->SteamGetEncryptionKeyToSendToNewClient( szEncryptionKey, &encryptionsize, sizeof(szEncryptionKey) ) 
			|| encryptionsize == 0 )
		{
...
void CBaseServer::UpdateMasterServerBasicData()
{
	...
	SteamGameServer()->SetMaxPlayerCount(nMaxReportedClients);
	SteamGameServer()->SetPasswordProtected((GetPassword() != NULL));
...
void CBaseServer::ForwardPacketsFromMasterServerUpdater()
{
	...
	while ( 1 )
	{
		...
		unsigned char packetData[16 * 1024];
 		int len = SteamGameServer()->GetNextOutgoingPacket( packetData, sizeof( packetData ), &netadrAddress, &netadrPort );
		if ( len <= 0 )
			break;
