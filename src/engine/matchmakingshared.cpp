...
CClientInfo *CMatchmaking::FindClient( netadr_t *adr )
{
	...
	unsigned int ip = adr->GetIPHostByteOrder();

	if ( ip == m_Host.m_adr.GetIPHostByteOrder() )
	{
		pClient = &m_Host;
	}
	else
	{
		...
		{
			if ( ip == m_Remote[i]->m_adr.GetIPHostByteOrder() )
			{
				pClient = m_Remote[i];
				break;
			}
...
{
	INetChannel *pNewChannel = FindChannel( adr->GetIPHostByteOrder() );
	if ( !pNewChannel )
	{
...
INetChannel *CMatchmaking::AddRemoteChannel( netadr_t *adr )
{
	...
	if ( pNetChannel )
	{
		...
		m_Channels.Insert( adr->GetIPHostByteOrder(), pNetChannel );
	}
	return pNetChannel;
}
...
{
	INetChannel *pNetChannel = FindChannel( adr->GetIPHostByteOrder() );
	if ( pNetChannel )
	{
		m_Channels.Remove( adr->GetIPHostByteOrder() );
	}
}
...
{
	m_ChannelsToRemove.AddToTail( adr->GetIPHostByteOrder() );
}
...
{
	INetChannel *pChannel = FindChannel( adr->GetIPHostByteOrder() );
	if ( pChannel )
	{
		Msg( "Setting new timeout for ip %d: %d\n", adr->GetIPHostByteOrder(), timeout );
...
{
	// Find the matching net channel
	INetChannel *pChannel = FindChannel( pClient->m_adr.GetIPHostByteOrder() );
	if ( pChannel )
	{
		pChannel->SendNetMsg( *msg, false, bVoice );
		if ( !pChannel->Transmit() )
...
{
	if ( pClient->m_adr.GetIPHostByteOrder() == 0 )
		return false;
